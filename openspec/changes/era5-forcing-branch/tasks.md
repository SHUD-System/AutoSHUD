...
- [ ] Add ERA5 configuration parsing in `Rfunction/ReadProject.R`.
...
- [ ] Add ERA5 branch dispatch in `Step2_DataSubset.R`.
  - Route `xfg$iforcing == 0.7` to the ERA5 converter.
  - Preserve all existing forcing branch behavior.
- [ ] Add ERA5 converter module, preferably `Rfunction/ERA5_NC2CSV.R`.
...
- [ ] Add ERA5 forcing documentation.
...
- [ ] `Rfunction/ReadProject.R` scenario: ERA5 config parsing.
...
- [ ] `Step2_DataSubset.R` scenario: config/branch dispatch path.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: selecting ERA5 grid points using watershed bbox plus buffer.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: `0-360` longitude handling.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: `-180..180` longitude handling.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: unit conversions for `tp`, `t2m`, `d2m`, `u10`, `v10`, `ssr`, and `sp`.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: cumulative `tp` forward differencing across day/file boundaries.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: cumulative `ssr` forward differencing across day/file boundaries.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: CSV output schema.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: meteo shapefile metadata.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: missing variable error.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: missing file error.
...
- [ ] `Rfunction/ERA5_NC2CSV.R` scenario: malformed time error.
...
- [ ] End-to-end scenario: classic Step1-Step3 ERA5 path.
